...
if [ -S /run/lldpd.socket ]; then
    ...
    sudo lldpcli configure system hostname "dc-ny-server" || echo "Note: lldpcli hostname configuration failed (non-critical)"
    sudo lldpcli configure system description "Ubuntu Server - dc-ny-server" || echo "Note: lldpcli description configuration failed (non-critical)"
else
    echo "Warning: lldpd socket not found, skipping lldpcli configuration"
    echo "You can manually configure later with: sudo lldpcli configure system hostname 'dc-ny-server'"
fi
...
# Configured for dc-ny-server with public community string
...
if [ "$current_hostname" != "dc-ny-server" ]; then
    echo "Setting hostname to dc-ny-server..."
    sudo hostnamectl set-hostname dc-ny-server
    echo "127.0.1.1    dc-ny-server" | sudo tee -a /etc/hosts > /dev/null
fi
...
echo "4. Hostname is set to 'dc-ny-server'"
